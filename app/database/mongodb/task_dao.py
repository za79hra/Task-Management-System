...
async def get_user_for_task_id_query(task_id: str):
    try:
        with MongoConnection() as mongo:
            data = mongo.tasks.find_one({"_id": ObjectId(task_id)})
        if data:
...
async def get_user_for_assigned_task_query(assigned_user_id: str):
    try:
        ...
        with MongoConnection() as mongo:
            data = list(mongo.tasks.find({"assigned_user_id": assigned_user_id}))
        if data:
            # print(data)
            return True, data
        return False, {'error': "not exist", 'status_code': 404}
...
async def update_task_for_new_assigned_query(task_id, task_data):
    try:
        with MongoConnection() as mongo:
            result = mongo.tasks.update_one({'_id': ObjectId(task_id)}, {'$set': task_data})
